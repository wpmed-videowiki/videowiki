...
export function capsToCamel(string) {
  const camelCaps = string.replace(
    /_?([A-Z]+)/g,
    (match, group) => `${group[0].toUpperCase()}${group.slice(1).toLowerCase()}`
  );
...
export function camelToCaps(string) {
  const underscoreCamel = string.replace(
    /([a-z])([A-Z])/g,
    (match, end, begin) => `${end}_${begin}`
  );
...
export function snakeToCamel(string) {
  return string.replace(/_([a-z])/g, (match, group) => group.toUpperCase());
}
...
export function url(strings, ...values) {
  const init = values.reduce(
    (result, value, index) =>
      `${result}${strings[index].trim()}${encodeURIComponent(value)}`,
    ""
  );
