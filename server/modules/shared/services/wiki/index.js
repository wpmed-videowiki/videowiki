...
export const getArticleMedia = function (title, wikiSource, callback) {
  ...
  getSectionWikiContent(title, wikiSource, (err, sections) => {
        ...
        if (mediaMatch && mediaMatch.length > 0) {
          ...
          mediaMatch.forEach(file => {
            ...
            const fileTitle = file
              .replace(
                FILE_PREFIXES[getLanguageFromWikisource(wikiSource)],
                FILE_PREFIXES['en']
              )
              .split('|')[0]
              .replace('[[', '')
              .trim()
              .replace(fileRegex, FILE_MATCH_REGEX['en'])
            if (!section.rawMedia) {
      ...
      mediaFuncArray.push(cb => {
        ...
        request.get(infoUrl, (err, res) => {
          ...
          try {
            ...
            // attemp missing pages on NC Commons
            const missingTitles = pages
              .filter(page => page.missing)
              .map(page => page.title)

            if (missingTitles.length > 0) {
              const ncCommonsInfoUrl = `https://nccommons.org/w/api.php?action=query&titles=${encodeURIComponent(
                missingTitles.join('|')
              )}&prop=imageinfo&iiprop=url|mediatype|size&iiurlwidth=${PLAYER_IMAGE_WIDTH}&format=json&formatversion=2`

              request.get(ncCommonsInfoUrl, (err, res) => {
                if (err) return cb(err)
                try {
                  const ncpages = JSON.parse(res.body).query.pages
                  ncpages
                    .filter(page => !page.missing)
                    .forEach(page => {
                      if (
    ...
    async.series(mediaFuncArray, err => {
      ...
      sections
        .filter(section => section.rawMedia && section.rawMedia.length > 0)
        .forEach(section => {
          if (!section.media) {
            section.media = []
          }
          section.rawMedia.forEach(rawMedia => {
            ...
            if (titleThumbnailMap[rawMedia]) {
              ...
              if (
                item.thumburl &&
                item.thumburl.indexOf(`${PLAYER_IMAGE_WIDTH}px-`)
              ) {
                item.smallThumb = item.thumburl.replace(
                  `${PLAYER_IMAGE_WIDTH}px-`,
                  `${PLAYER_THUMB_WIDTH}px-`
                )
              }
...
export const fetchArticleRevisionId = function fetchArticleVersion (
  title,
  wikiSource,
  callback
) {
  const url = `${wikiSource}/w/api.php?action=query&format=json&prop=revisions&titles=${encodeURIComponent(
    title
...
export const generateDerivativeTemplate = function generateDerivativeTemplate (
  derivative
) {
  return `{{collapse top|{{Template:Derived_from\n|1=${decodeURIComponent(
    derivative.fileName
  )}|by=${derivative.author}\n}}}}\n{{${
    derivative.licence
  }}}\n{{collapse bottom}}`
}

export const fetchArticleContributors = function (
  title,
  wikiSource,
  callback = () => {}
) {
    ...
    try {
      ...
      Object.keys(body.query.pages).forEach(pageId => {
        contributors = contributors.concat(
          body.query.pages[pageId].contributors
        )
      })
...
function extractSectionsFromText (title, sections, text, wikiSource) {
  ...
  for (let i = 1; i <= sections.length; i++) {
    ...
    if (
      SECTIONS_BLACKLIST[VIDEOWIKI_LANG] &&
      SECTIONS_BLACKLIST[VIDEOWIKI_LANG].some(
        s =>
          previousSectionTitle.toLowerCase().trim() === s.toLowerCase().trim()
      )
    ) {
      //
    } else {
...
export function getSectionText (wikiSource, title, callback) {
  getTextFromWiki(wikiSource, title, (err, text) => {
    ...
    getSectionsFromWiki(wikiSource, title, (err, sections) => {
      ...
      const updatedSections = extractSectionsFromText(
        title,
        sections,
        text,
        wikiSource
...
export function fetchArticleSectionsReadShows (
  title,
  wikiSource,
  callback = () => {}
) {
...
export function fetchTitleRedirect (title, wikiSource, callback = () => {}) {
  const url = `${wikiSource}/w/api.php?action=query&titles=${encodeURIComponent(
    title
